...
function controlFeed() {
  ...
  if (feedLength < newFeedLength) {
    ...
    // insert content when cum difference is signifcant not diff
    cum = cum + difference
    ...
    if (cum > n) {
      ...
      for (i = 0; i < times; i++) {
        ...
        //start inserting content from the last position
        insertContent(pointer + n * i, feedContent)
        console.log(`insert content: ${pointer + n * i}`)
        if (i === times - 1) {
          pointer = pointer + n * times
        }
      }
      //reset cum
      cum = 0
